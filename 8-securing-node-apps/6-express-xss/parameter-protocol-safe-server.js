...
function validateParameters({ previous, lang, token }, query) {
  return (
...
app.get('/', (req, res) => {
  ...
  if(!validateParameters(req.query, req.query)) {
    res.status(422).end();
    return;
  }
...
// http://localhost:3000/?previous=sixteencharacters&token=sixteencharacters&lang=en
